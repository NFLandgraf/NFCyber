...
def get_files(path):
    ...
    files = [file for file in Path(path).iterdir() if file.is_file() and '_Y' in file.name]
...
for video_in in files:
    print(video_in)

    video_out = f"{video_in.stem}_mask.mp4"
    video_out = output_path / video_out
    adjust_video(video_in, video_out)
