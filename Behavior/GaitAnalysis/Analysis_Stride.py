...
def cleaning_raw_df(df, nan_threshold=0.3):
    ...
    return df
def add_basics_columns(df):

    # add distances beetween bps    
    df['dist_H_paws'] = euclidian_dist(df['paw_HL_x'].values, df['paw_HL_y'].values, df['paw_HR_x'].values, df['paw_HR_y'].values)
    df['dist_V_paws'] = euclidian_dist(df['paw_VL_x'].values, df['paw_VL_y'].values, df['paw_VR_x'].values, df['paw_VR_y'].values)
    df['dist_L_paws'] = euclidian_dist(df['paw_HL_x'].values, df['paw_HL_y'].values, df['paw_VL_x'].values, df['paw_VL_y'].values)
    df['dist_R_paws'] = euclidian_dist(df['paw_HR_x'].values, df['paw_HR_y'].values, df['paw_VR_x'].values, df['paw_VR_y'].values)
...
def get_strides(df):
    ...
    def paw_speed_minmax(df, nec_speed_on=80, nec_speed_off=80):
        ...
        return steps_dict
    steps_dict = paw_speed_minmax(df, nec_speed_on=80, nec_speed_off=80)
...
def calc_stride_vectors(df, stride_segments):
    ...
    for start, end in stride_segments:
        x1 = np.mean(df.loc[start-1 : start+1, 'anus_x'])
        y1 = np.mean(df.loc[start-1 : start+1, 'anus_y'])
        x2 = np.mean(df.loc[end-1 : end+1, 'anus_x'])
        y2 = np.mean(df.loc[end-1 : end+1, 'anus_y'])
...
def lateral_displacement(df, stride_segments, stride_vectors, bodypart, num_bins=20):
    ...
    for i, stride in enumerate(stride_segments):
        ...
        for frame in range(stride[0], stride[1]+1):
            distance = point2line_dist(slope, intercept, df.loc[frame, f'{bodypart}_x'], df.loc[frame, f'{bodypart}_y'])
    ...
    # bin the displacements into evenly spaced bins and compute mean displacement for each bin
...
for file in file_list:
    ...
    plot_average_stride(output_name, norm_strides_dict, mean_strides_dict, ['paw_VL', 'paw_HR', 'paw_HL', 'paw_VR'])

    # lateral stuff
    for bp in ['snout', 'mouth', 'tail_start', 'tail_middle', 'tail_end', 'anus']:
...
#%%
anim_bodyparts = ['snout', 'paw_VL','paw_VR', 'paw_HR', 'paw_HL', 'anus']

def animation(df, bodypart_list, stride_segment):

    frames = np.arange(stride_segment[0], stride_segment[1], 1)

    def get_min_max_coordinates(df, bodyparts, rows):

        df_subset = df.loc[rows]

        # Extract x and y columns for the specified body parts
        x_columns = [f"{bp}_x" for bp in bodyparts]
        y_columns = [f"{bp}_y" for bp in bodyparts]

        # Compute min and max for x and y coordinates across the subset
        extra = 10
        min_x = df_subset[x_columns].min().min() - extra
        max_x = df_subset[x_columns].max().max() + extra
        min_y = df_subset[y_columns].min().min() - extra
        max_y = df_subset[y_columns].max().max() + extra

        return min_x, max_x, min_y, max_y
    min_x, max_x, min_y, max_y = get_min_max_coordinates(df, bodypart_list, frames)

    colors = ['gray', 'b', 'c', 'orange', 'r', 'gray']
    fig, ax = plt.subplots()
    ...
    #anim.save('continuousSineWave.mp4',  writer = 'ffmpeg', fps = 10) 
    plt.show()
    return anim

my_animation = animation(main_df, anim_bodyparts, stride_segments[0])
